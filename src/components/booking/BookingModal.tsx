...
const BOOKING_URL = 'https://n8n.halovisionai.cloud/webhook/halovisionschedule880088'
    ...
    fetch(BOOKING_URL, {
      ...
      .then(data => {
        ...
        if (data) {
          ...
          if (aiResponse) aiResponse = aiResponse.replace(/<[^>]*>/g, '').replace(/\*\*/g, '').trim()
        }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center md:p-6">
      ...
      <motion.div
        ...
        style={{ background: 'radial-gradient(ellipse 100% 100% at 50% 50%, rgba(80,30,10,0.55) 0%, rgba(0,0,0,0.75) 70%)' }}
        onClick={onClose}
      />
      ...
      >
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 h-px rounded-full pointer-events-none"
            style={{ background: 'linear-gradient(90deg, transparent 5%, rgba(200,120,40,0.45) 40%, rgba(255,160,60,0.55) 55%, rgba(200,120,40,0.45) 70%, transparent 95%)' }}
          />
          ...
          <div className="flex items-start justify-between px-6 py-5 border-b border-white/[0.06]">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] text-amber-400/60 tracking-[0.25em] uppercase font-medium">Halo AI</span>
              </div>
            ...
            <button data-cursor="hover" onClick={onClose} className="text-white/25 hover:text-white/55 transition-colors mt-0.5">
              <X className="w-4 h-4" />
            </button>
          </div>

          {/* Progress — glowing segments */}
          <div className="px-6 py-3 flex items-center gap-0 border-b border-white/[0.04]">
            {stepLabels.map((label, i) => (
              <div key={i} className="flex items-center gap-0 flex-1 last:flex-none">
                <div className="flex items-center gap-1.5 shrink-0">
                  <div
                    className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-medium transition-all duration-400"
                    style={i < currentStepIdx
                      ? { background: 'rgba(251,146,60,0.25)', border: '1px solid rgba(251,146,60,0.55)', color: 'rgba(251,191,36,0.9)', boxShadow: '0 0 8px rgba(251,146,60,0.3)' }
                      : i === currentStepIdx
                      ? { background: 'rgba(255,165,60,0.12)', border: '1px solid rgba(255,165,60,0.50)', color: 'rgba(255,255,255,0.85)' }
                      : { background: 'transparent', border: '1px solid rgba(255,255,255,0.10)', color: 'rgba(255,255,255,0.22)' }
                    }
                  >
                    {i < currentStepIdx ? '✓' : i + 1}
                  </div>
                  <span className={`text-[10px] tracking-wide hidden sm:block transition-all ${i === currentStepIdx ? 'text-white/55' : i < currentStepIdx ? 'text-white/35' : 'text-white/18'}`}>
                    {label}
                  </span>
                </div>
                {i < stepLabels.length - 1 && (
                  <div className="flex-1 mx-2 h-px transition-all duration-400"
                    style={{ background: i < currentStepIdx ? 'rgba(251,146,60,0.40)' : 'rgba(255,255,255,0.06)' }}
                  />
                )}
              </div>
            ))}
                  ...
                  {isSuccess ? (
                    <div className="flex flex-col items-center gap-6 py-10 text-center">
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            background: 'linear-gradient(135deg, rgba(220,105,20,0.22) 0%, rgba(180,80,10,0.16) 100%)',
                            border: '1px solid rgba(255,150,50,0.36)',
                            boxShadow: '0 0 16px rgba(220,100,20,0.14), inset 0 1px 0 rgba(255,200,100,0.12)',
                          }}
                          onMouseEnter={e => {
                            (e.currentTarget as HTMLButtonElement).style.background = 'linear-gradient(135deg, rgba(220,105,20,0.32) 0%, rgba(180,80,10,0.24) 100%)'
                          }}
                          onMouseLeave={e => {
                            (e.currentTarget as HTMLButtonElement).style.background = 'linear-gradient(135deg, rgba(220,105,20,0.22) 0%, rgba(180,80,10,0.16) 100%)'
                          }}
                        >
                    ...
                    <>
                      ...
                      <div className="flex gap-3">
                        <button data-cursor="hover" onClick={() => setStep('form')}
                        ...
                        <button
                          ...
                          style={{
                            background: 'linear-gradient(135deg, rgba(220,105,20,0.28) 0%, rgba(180,80,10,0.20) 100%)',
                            border: '1px solid rgba(255,150,50,0.42)',
                            boxShadow: '0 0 20px rgba(220,100,20,0.18), inset 0 1px 0 rgba(255,200,100,0.15)',
                          }}
                        >
